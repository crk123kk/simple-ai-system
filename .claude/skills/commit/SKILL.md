...
仅 `feat` 和 `fix` 类型的提交支持添加任务/缺陷 ID。

**提取优先级：**

1. **用户消息**：从用户的提交请求中匹配数字 ID（如 `1098399`、`#1098399`）
2. **分支名**：从当前分支名中提取，匹配 `fix_1146140`、`feat/1098399`、`fix-1098399` 等模式，取分隔符（`_`、`/`、`-`）后的纯数字部分

**处理规则：**

- 提取到 ID 后，告知用户并确认是否添加到提交信息中
- 未提取到 ID 时，沿用现有格式，不主动询问
- 非 `feat`/`fix` 类型的提交不添加 ID，即使提取到也忽略
...
| 场景 | 格式 | 示例 |
| --- | --- | --- |
| feat/fix + 有 ID | `<type>: <emoji> [<ID>] <中文描述>` | `fix: 🐛 [1146140] 修复切换对话时 loading 闪烁问题` |
| feat/fix + 无 ID | `<type>: <emoji> <中文描述>` | `feat: ✨ 新增用户认证系统` |
| 其他类型 | `<type>: <emoji> <中文描述>` | `refactor: ♻️ 简化错误处理逻辑` |
